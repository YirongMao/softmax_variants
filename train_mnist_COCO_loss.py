...
def visualize(feat, labels, epoch):
    ...
    plt.savefig('./images/coco_loss_epoch=%d.jpg' % epoch)
    # plt.draw()
    # plt.pause(0.001)
    plt.close()
...
def main():
    ...
    coco_loss = model_utils.COCOLoss(10, 2)
    if use_cuda:
        nllloss = nllloss.cuda()
        coco_loss = coco_loss.cuda()
        model = model.cuda()
    criterion = [nllloss, coco_loss]
    # optimzer4nn
    optimizer4nn = optim.SGD(model.parameters(), lr=0.001, momentum=0.9, weight_decay=0.0005)
    sheduler_4nn = lr_scheduler.StepLR(optimizer4nn, 10, gamma=0.5)

    # optimzer4center
    optimzer4center = optim.SGD(coco_loss.parameters(), lr=0.01, momentum=0.9)
    # sheduler_4center = lr_scheduler.StepLR(optimzer4center, 10, gamma=0.5)
    for epoch in range(100):
        sheduler_4nn.step()
        # sheduler_4center.step()
        # lr = sheduler_4center.get_rate(epoch)
        # print optimizer4nn.param_groups[0]['lr']
        train(train_loader, model, criterion, [optimizer4nn, optimzer4center], epoch + 1, use_cuda)
        torch.save(model, './model/coco_loss_net_' + str(epoch + 1) + '.model')
