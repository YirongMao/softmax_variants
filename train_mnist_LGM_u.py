...
def train(train_loader, model, criterion, optimizer, epoch, loss_weight, use_cuda):
    ...
    for i, (data, target) in enumerate(train_loader):
        ...
        if (i + 1) % 50 == 0:
            tmp = criterion[1].log_covs
            norm = torch.sum(torch.mul(tmp, tmp))
            print('Epoch [%d], Iter [%d/%d] Cov_norm %.4f Loss: %.4f Acc %.4f'
                  % (epoch, i + 1, len(train_loader) // batch_size, norm.data[0], loss.data[0], accuracy))
...
def main():
    ...
    # optimzer4center
    optimzer4center = optim.SGD(lgm_loss.parameters(), lr=0.01, momentum=0.9)
    sheduler_4center = lr_scheduler.StepLR(optimizer4nn, 10, gamma=0.5)
    for epoch in range(100):
        ...
        torch.save(model, './model/LGM_u_net_' + str(epoch + 1) + '.model')
